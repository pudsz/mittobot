...
### Problem
The dashboard (Vite on port 5173) made direct cross-origin `fetch()` calls to the bot's API (port 3432). This could fail due to:
...
### Solution
- **`dashboard/vite.config.js`** — Added `server.proxy` to forward `/api/*` and `/login` to `http://0.0.0.0:3432`
- **`scripts/dev.js`** — Removed `VITE_BOT_API_URL` env var passthrough — the SPA now uses same-origin requests through Vite's proxy
- The proxy reads `VITE_BOT_API_URL` with fallback to `http://0.0.0.0:3432`, so it's still configurable
