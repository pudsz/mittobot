...
module.exports = [
  ...
  },
  {
    ...
    prefix: async (m) => { try { m.reply({ embeds: [await meme()] }); } catch { m.reply({ embeds: [apiErrorEmbed("a meme")] }); } },
    slash: new SlashCommandBuilder().setName("meme").setDescription("Random meme"),
    execute: async (i) => { await i.deferReply(); try { i.editReply({ embeds: [await meme()] }); } catch { i.editReply({ embeds: [apiErrorEmbed("a meme")] }); } },
  },
  {
    name: "joke", description: "Random joke", category: CATEGORY,
    prefix: async (m) => { try { m.reply({ embeds: [await joke()] }); } catch { m.reply({ embeds: [apiErrorEmbed("a joke")] }); } },
    slash: new SlashCommandBuilder().setName("joke").setDescription("Random joke"),
    execute: async (i) => { await i.deferReply(); try { i.editReply({ embeds: [await joke()] }); } catch { i.editReply({ embeds: [apiErrorEmbed("a joke")] }); } },
  },
  {
    name: "dadjoke", description: "Random dad joke", category: CATEGORY,
    prefix: async (m) => { try { m.reply({ embeds: [await dadjoke()] }); } catch { m.reply({ embeds: [apiErrorEmbed("a dad joke")] }); } },
    slash: new SlashCommandBuilder().setName("dadjoke").setDescription("Random dad joke"),
    execute: async (i) => { await i.deferReply(); try { i.editReply({ embeds: [await dadjoke()] }); } catch { i.editReply({ embeds: [apiErrorEmbed("a dad joke")] }); } },
  },
  {
    name: "cat", description: "Random cat picture", category: CATEGORY,
    prefix: async (m) => { try { m.reply({ embeds: [await cat()] }); } catch { m.reply({ embeds: [apiErrorEmbed("a cat")] }); } },
    slash: new SlashCommandBuilder().setName("cat").setDescription("Random cat picture"),
    execute: async (i) => { await i.deferReply(); try { i.editReply({ embeds: [await cat()] }); } catch { i.editReply({ embeds: [apiErrorEmbed("a cat")] }); } },
  },
  {
